...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. PREDATOR AVOIDANCE (v7 parameters)
    if predators:
    ...
    # 3. TREE FORAGING (v7 parameters)
    elif nearest_tree is not None and energy_ratio < 0.82:
    ...
    # 5. REPRODUCTION (v7 safe parent energy reserve)
    if not predators and energy >= 190.0 and age >= 12.0:
        spawn_agent = True
