...
def weighted_escape_direction(predators: list[dict], other_agents: list[dict]) -> float:
    """
    Calculate weighted escape direction away from all observed predators and de-clump from other agents.
    """
    escape_x = 0.0
    escape_y = 0.0
    ...
    for predator in predators:
        ...
        weight = 15000.0 / (distance * distance + 1.0)
    ...
    for agent_obs in other_agents:
        ...
        if distance < 100.0:
            ...
            weight = 400.0 / (distance * distance + 5.0)
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    if predators:
        ...
        # Sprint threshold: sprint whenever predator is within 80 units
        if nearest_pred_dist <= 80.0 and energy > 15.0:
            move_distance = sprint_speed
        else:
            move_distance = speed

        # Look towards nearest predator to track while fleeing
    ...
    # 3. Tree Foraging (where fruits spawn)
    elif nearest_tree is not None and energy_ratio < 0.90:
    ...
    # 4. Active Exploration & Vision Sweep
        ...
        if abs(wx) > 1e-9 or abs(wy) > 1e-9:
            ...
            turn_angle = 0.06 * math.sin(age * 0.3 + phase)

    # 5. Hyper-Evolutionary High-Yield Reproduction Strategy
    # Cost: 100 energy. Spawn whenever energy >= 135.0 and no predator < 75 units!
    near_predators = [p for p in predators if float(p["distance"]) < 75.0]
    if not near_predators and energy >= 135.0 and age >= 8.0:
        spawn_agent = True
