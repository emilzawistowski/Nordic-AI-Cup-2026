...
Fix in v44:
- Maintain exact v11 movement, escaping, wall deflection, and foraging.
- Only spawn when energy >= 185 AND age >= 10 (v11 baseline).
- Add dynamic threshold: in high predator count (>= 3 visible), require energy >= 210
  so parent has even more reserve buffer when surroundings are dangerous.
"""
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. Predator Avoidance (Exact v11)
    if predators:
    ...
    # 4. Economical Wandering (Exact v11)
    else:
    ...
    # 5. Reproduction Guard: require higher reserve in dangerous situations
    required_energy = 210.0 if len(predators) >= 2 else 185.0
    if not predators and energy >= required_energy and age >= 10.0:
        spawn_agent = True
