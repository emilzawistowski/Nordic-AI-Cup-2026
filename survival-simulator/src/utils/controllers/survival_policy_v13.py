...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. Highest Priority: Unconditional Emergency Predator Evasion & Pivot Lock
    if predators:
        ...
        # Extended Sprint Horizon: sprint whenever predator <= 75u and energy > 12u
        if nearest_pred_dist <= 75.0 and energy > 12.0:
            move_distance = sprint_speed
        else:
            move_distance = speed

        # Look TOWARDS nearest predator to force predator into pivot mode
        nearest_p = min(predators, key=lambda p: float(p["distance"]))
        pred_angle = float(nearest_p["angle"])
        turn_angle = max(-0.22, min(0.22, pred_angle))
