...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    if predators:
        ...
        # Sprint threshold: sprint if predator is within 65 units
        if nearest_pred_dist <= 65.0 and energy > 20.0:
    ...
    # 5. Adaptive Reproduction Strategy
    # Cost: 100. Spawn threshold depends on local agent density.
    local_population = len(other_agents)
    spawn_threshold = 155.0 if local_population <= 2 else 175.0
