...
With aggressive spawning, we build population FAST. In a population of 10+,
we expect 1 elite runner per generation (speed/sprint_speed mutations).
Fast agents (sprint_speed 30-40) can easily outrun predators (sprint = 15).

Strategy:
- Spawn whenever energy > 165 (aggressively, even in slight predator presence)
- Focus on fruit harvesting to maintain energy for constant spawning
- Standard v11 escape for predators
- Accept more spawn-starvation risk early in exchange for population explosion
"""
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 2. Fruit Harvesting
        ...
        # Sprint toward fruit if energy is high (to eat more fruits faster and spawn sooner)
        if energy_ratio > 0.90 and fruit_dist > 20.0:
            move_distance = sprint_speed  # sprint to eat quickly and spawn
        else:
            move_distance = min(speed, fruit_dist)
        turn_angle = max(-0.10, min(0.10, fruit_angle))

    # 3. Tree Foraging
    elif nearest_tree is not None and (energy_ratio < 0.90 or age > 60.0):
    ...
    # 5. Aggressive Breeding Strategy (low threshold = 165)
    # Spawn more often to build population faster → more chance of elite speed mutations
    if not predators and energy >= 165.0 and age >= 8.0:
        spawn_agent = True
