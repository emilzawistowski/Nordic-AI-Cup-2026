...
def get_tree_cluster_centroid(trees: list[dict]) -> tuple[float, float, int]:
    """Calculate centroid angle, distance, and count of visible tree cluster."""
    if not trees:
        return 0.0, 0.0, 0
    cx = 0.0
    cy = 0.0
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    if predators:
        ...
        # Trait-Aware Speed Selection: fast mutated agents walk, others sprint
        if speed >= 13.5:
            move_distance = speed
    ...
    # 2. Fruit Harvesting (Best energy/distance ratio)
    elif fruits:
