...
Fix:
- Sprint ONLY when predator is within 45.0 units (very close, immediate danger). 
- At 45-80 units: walk at 100% speed away.
- At > 80 units: walk at 70% speed (conserve energy, let predator approach to visual range where we can plan better).
- Maintain v11's reproduction (185u) and 6000/(dist^2+1) escape weighting.
"""
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. PREDATOR AVOIDANCE - Ultra-conservative sprinting
    if predators:
        ...
        # Hard sprint floor: only sprint at < 45 units
        if nearest_pred_dist <= 45.0 and energy > 20.0:
            move_distance = sprint_speed
        elif nearest_pred_dist <= 80.0:
            move_distance = speed  # full walk when moderate threat
        else:
            move_distance = speed * 0.70  # conserve energy when far
