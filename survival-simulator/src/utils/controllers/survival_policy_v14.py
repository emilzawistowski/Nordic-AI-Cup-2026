...
def inverse_cubic_escape_direction(predators: list[dict]) -> float:
    ...
    for predator in predators:
        ...
        # Inverse cubic force field weighting for strong close-range repulsion
...
def get_best_fruit(fruits: list[dict]) -> dict:
    return max(
        fruits,
        key=lambda f: float(f.get("energy", 50.0)) / (float(f["distance"]) * float(f["distance"]) + 10.0)
    )
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. Highest Priority: Inverse-Cubic Predator Avoidance & Trait-Aware Evasion
    if predators:
        nearest_pred_dist = min(float(p["distance"]) for p in predators)
        move_direction = inverse_cubic_escape_direction(predators)
        ...
        # If agent inherited high mutated speed (speed >= 13.5), walking at speed outruns predators at 4x lower energy cost!
        if speed >= 13.5:
            move_distance = speed
        elif nearest_pred_dist <= 65.0 and energy > 15.0:
            move_distance = sprint_speed
        else:
            move_distance = speed

        # Look TOWARDS nearest predator to lock predator in pivot mode
    ...
    # 5. Champion Reproduction Strategy
    # Cost: 100. Require energy >= 180.0 so parent keeps >= 80 energy.
    if not predators and energy >= 180.0 and age >= 10.0:
        spawn_agent = True
