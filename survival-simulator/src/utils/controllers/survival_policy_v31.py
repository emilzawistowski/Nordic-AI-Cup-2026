...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 2. FRUIT HARVESTING (with selective sprint catch-up)
        ...
        # Sprint toward distant fruit only when energy is high (>= 75%)
        if fruit_dist > 80.0 and energy_ratio >= 0.75:
            move_distance = min(sprint_speed, fruit_dist)
        else:
