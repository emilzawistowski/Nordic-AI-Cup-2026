...
def weighted_escape_direction(predators: list, nearby_agents: list) -> float:
    escape_x = 0.0
    escape_y = 0.0
    ...
    # Gentle repulsion from sibling agents (spread out population)
    for agent in nearby_agents:
        distance = max(float(agent.get("distance", 1.0)), 1.0)
        if distance < 40.0:
            agent_angle = float(agent.get("angle", 0.0))
            repel_angle = normalize_angle(agent_angle + math.pi)
            weight = 500.0 / (distance * distance + 1.0)
            escape_x += math.cos(repel_angle) * weight
            escape_y += math.sin(repel_angle) * weight
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    if predators:
        ...
        move_direction = weighted_escape_direction(predators, nearby_agents)
    ...
    # 4. ECONOMICAL WANDERING (with gentle sibling dispersal)
    else:
        phase = agent_id * 1.61803398875
        slow_wave = math.sin(age * 0.055 + phase)
        faster_wave = math.sin(age * 0.017 + phase * 0.7)
        base_dir = 0.50 * slow_wave + 0.20 * faster_wave

        # Add light dispersal from nearby agents when wandering
        disp_x, disp_y = math.cos(base_dir), math.sin(base_dir)
        for a in nearby_agents:
            d = max(float(a.get("distance", 1.0)), 1.0)
            if d < 35.0:
                a_angle = float(a.get("angle", 0.0))
                disp_x += math.cos(normalize_angle(a_angle + math.pi)) * (35.0 - d) / 35.0
                disp_y += math.sin(normalize_angle(a_angle + math.pi)) * (35.0 - d) / 35.0

        move_direction = normalize_angle(math.atan2(disp_y, disp_x))
    ...
    # 5. REPRODUCTION (v11 Baseline Parameters)
