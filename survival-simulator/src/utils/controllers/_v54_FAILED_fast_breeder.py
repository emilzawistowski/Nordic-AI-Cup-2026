...
# Speed-only colony registry; remaining containers are expiry/reset bookkeeping.
COLONY = {}
_COLONY_LAST_SEEN = {}
_COLONY_LAST_AGE = {}
_COLONY_SEEN = set()
...
def reset_colony():
    global _COLONY_TICK
    COLONY.clear()
    _COLONY_LAST_SEEN.clear()
    _COLONY_LAST_AGE.clear()
    _COLONY_SEEN.clear()
    _COLONY_TICK = 0


def update_colony(agent_id, speed, age):
    global _COLONY_TICK
    if agent_id in _COLONY_LAST_AGE and age < _COLONY_LAST_AGE[agent_id]:
        reset_colony()
    if agent_id in _COLONY_SEEN:
        _COLONY_TICK += 1
        _COLONY_SEEN.clear()
    _COLONY_SEEN.add(agent_id)
    COLONY[agent_id] = speed
    _COLONY_LAST_SEEN[agent_id] = _COLONY_TICK
    _COLONY_LAST_AGE[agent_id] = age
    for stale_id in list(COLONY):
        if _COLONY_TICK - _COLONY_LAST_SEEN[stale_id] >= 2:
            del COLONY[stale_id]
            del _COLONY_LAST_SEEN[stale_id]
            del _COLONY_LAST_AGE[stale_id]
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    update_colony(agent_id, speed, age)
    ...
    if predators:
        ...
        # Sprinting decision: sprint whenever predator is within 55 units and energy > 20
        real_floor = max_energy / 5.0
        if speed >= 15.5:
            move_distance = speed
        elif nearest_pred_dist <= 55.0 and energy > real_floor:
    ...
    # Cost: 100. Spawn when energy >= 185.0 and age >= 10.0
    if not predators and energy >= 185.0 and age >= 10.0:
        if len(COLONY) < 10:
            spawn_agent = True
        else:
            best = max(COLONY.values())
            if speed >= 0.97 * min(best, 19.5):
                spawn_agent = True
