...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    if predators:
        ...
        # Sprint when close or when threatened by multiple predators
        if nearest_pred_dist <= 55.0 and energy > 20.0:
            move_distance = sprint_speed
        elif nearest_pred_dist <= 65.0 and len(predators) >= 2 and energy > 35.0:
            move_distance = sprint_speed
        else:
            move_distance = speed
