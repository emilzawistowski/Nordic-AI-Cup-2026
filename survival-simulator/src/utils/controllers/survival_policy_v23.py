...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. PREDATOR AVOIDANCE (Trait-Aware)
    if predators:
        ...
        # Dynamic sprint distance based on sprint capability
        sprint_trigger_dist = 55.0 + max(0.0, (sprint_speed - 20.0) * 0.4)
        if nearest_pred_dist <= sprint_trigger_dist and energy > 25.0:
    ...
    # 3. TREE FORAGING (Golden Ratio Angle Distribution)
        ...
        approach_offset = (agent_id * 0.61803398875) % 1.0 - 0.5
        move_direction = normalize_angle(tree_angle + approach_offset * 0.8)
