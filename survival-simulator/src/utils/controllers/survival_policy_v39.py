...
Strategy: SPRINT TO EVERY FRUIT SEEN. The faster we eat fruit, the more fruit
bonus accumulates AND the more energy we have to spawn more agents.

Also: maintain v11's proven escape logic.
"""
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # 1. Predator Avoidance — v11 proven logic
    if predators:
    ...
    # 2. AGGRESSIVE FRUIT SPRINT — always sprint to visible fruit
        ...
        # Sprint to fruit if we have energy AND it's worth going for
        if energy_ratio > 0.25 and fruit_dist > 5.0:
            move_distance = sprint_speed  # ALWAYS sprint to fruit
        else:
    ...
    # 5. Reproduction — v11 threshold (185u)
