...
def weighted_escape_direction(predators: list) -> float:
    ...
    for predator in predators:
        ...
        weight = 5000.0 / (distance * distance + 1.0)
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    if predators:
        ...
        # Sprint guardrail (only sprint when energy > 40.0)
        if nearest_pred_dist <= 50.0 and energy > 40.0:
    ...
    # 5. REPRODUCTION (High energy reserve)
    if not predators and energy >= 210.0 and age >= 15.0:
        spawn_agent = True
