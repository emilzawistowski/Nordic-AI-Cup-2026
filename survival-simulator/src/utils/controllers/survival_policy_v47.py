...
Synthesizing v11, v44, and v22 into v47:
- Flee escape weight: 7500.0 / (dist^2 + 1) (blend of v11's 6000 and v22's 10000)
- Flee edge deflection: 38.0u trigger with smoother smoothstep deflection angle
- Dynamic spawn reserve: 195.0 energy when len(predators) >= 2, 180.0 otherwise
- Wandering: frequency phase adjusted for zero-stutter turning
"""
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
            ...
            if coords and len(coords) == 2:
                ...
                if edge_dist < 38.0:
                    ...
                    if abs(diff) < math.pi / 2.0:
                        move_direction = normalize_angle(move_direction + math.copysign(math.pi / 2.4, diff))
    ...
    # 5. Hybrid Spawn Reserve
    spawn_threshold = 195.0 if len(predators) >= 2 else 180.0
