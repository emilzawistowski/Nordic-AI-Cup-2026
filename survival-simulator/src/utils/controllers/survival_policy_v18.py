...
Key insight from predator.py line 37:
  ...
  = angle of predator in OUR local frame = our predator["angle"]

  So: if abs(predator["angle"]) < pi/2 → we ARE facing the predator → predator PIVOTS
      if abs(predator["angle"]) >= pi/2 → we're not facing → predator CHARGES!

Strategy:
  - Always TURN to face the nearest predator (keep it in our forward cone)
  - Move PERPENDICULAR to the predator while facing it → predator pivots around us (inefficient)
  - This lets us escape while predator wastes time circling instead of closing distance
"""
...
def action_decision(observation_response: dict, rng: random.Random) -> ActionRequest:
    ...
    # -----------------------------------------------------------------------
    # 1. PREDATOR AVOIDANCE — "Face and Sidestep" Strategy
    # -----------------------------------------------------------------------
    if predators:
        ...
        # KEY TRICK: Check if we're facing the predator
        # If |predator_angle| < pi/2 → predator in pivot mode (circles us)
        # If |predator_angle| >= pi/2 → predator charges! Turn to face!
        facing_predator = abs(nearest_pred_angle) < math.pi / 2

        if facing_predator:
            # Predator is in PIVOT mode — we're exploiting this!
            # Move PERPENDICULAR to predator direction to slide away
            perp_cw  = normalize_angle(nearest_pred_angle - math.pi / 2)
            perp_ccw = normalize_angle(nearest_pred_angle + math.pi / 2)

            # Pick perp direction that aligns best with pure escape vector
            ...
            if dot_cw > dot_ccw:
                move_direction = perp_cw
            else:
                move_direction = perp_ccw

            # Gentle turn to maintain facing (keep predator in front half-plane)
            turn_angle = max(-0.12, min(0.12, nearest_pred_angle * 0.25))

        else:
            # NOT facing predator → it will CHARGE! Immediately turn to face
            move_direction = raw_escape
            # Turn aggressively toward predator to activate pivot mode
            turn_angle = max(-0.30, min(0.30, nearest_pred_angle * 0.7))

        # Wall-bounce: deflect when near boundary
        ...
        # Sprint decision
        if nearest_pred_dist <= 60.0 and energy > 25.0:
            move_distance = sprint_speed
        elif nearest_pred_dist <= 100.0 and energy > 40.0:
            move_distance = speed
        else:
    ...
    # -----------------------------------------------------------------------
    # 3. TREE FORAGING (age-compensated)
    # -----------------------------------------------------------------------
