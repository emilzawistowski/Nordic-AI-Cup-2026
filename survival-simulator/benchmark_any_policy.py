...
def run_simulation(seed: int, action_decision_fn) -> dict:
    ...
    while True:
        ...
        for agent, agent_state in zip(sim.env.agents, state["observations"]):
            action = action_decision_fn(agent_state, action_rng)
...
def main():
    ...
    parser.add_argument("--policy", type=str, default="src.utils.controllers.survival_policy_v3")
    parser.add_argument("--seeds", nargs="*", type=int, default=DEFAULT_SEEDS)
    args = parser.parse_args()

    module = importlib.import_module(args.policy)
    action_decision_fn = module.action_decision

    pygame.init()
    results = []

    print(f"--- Running Benchmark for Policy: {args.policy} ---")
    for index, seed in enumerate(args.seeds, start=1):
        result = run_simulation(seed, action_decision_fn)
