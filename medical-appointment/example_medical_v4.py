"""Experimental entry point for attempt 5 (hybrid ID + shrink).

Copy of example.py with imports switched to the v2/v4 files:
- medical_reasoner_v2.answer_questions_batch_indexed (sentence-ID citations)
- medical_evidence_v4.build_sentences / build_numbered_transcript /
  calibrate_indexed_span (F1 shrink within cited range + 0.3s start shift,
  fallback to full cited range)

Served for evaluation via api_v4.py on port 9056 so example.py is untouched.
"""
...
# Model preload at import (same hardening as example.py).
try:
